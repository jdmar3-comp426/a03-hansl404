...
export const filter = (arr, test) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        let check = arr[i]
        if (test(check)) result.pass.push(check)
        if (!test(check)) result.fail.push(check)
    }
    return result
};
...
export const allEvensAreOdd = (arr) => {
    return everyEven(arr, element => element % 2 == 1)
};
...
export const anEvenIsOdd = (arr) => {
    return someEven(arr, element => element % 2 == 1)
};
...
export const hasExactly = (arr, test, n) => {
    ...
    return result.pass.length == n ? true : false
};
